...
def perc_test(feat_vec, labeled_list, feat_list, tagset, default_tag):
    ...
    for i in range(2, N-2):
        ...
        for tag in tagset:
            ...
            prev_list = []
            pprev_list = []
            for pprev_tag in viterbi[i-2]:
                (pprev_value, pprev_backpointer1, pprev_backpointer2) = viterbi[i-2][pprev_tag]
                pprev_tag_weight = weight

                for prev_tag in viterbi[i-1]:
                    #print >>sys.stderr, "word:", word, "feat:", feat, "tag:", tag, "prev_tag:", prev_tag
                    (prev_value, prev_backpointer1, prev_backpointer2) = viterbi[i-1][prev_tag]
                    ...
                    if has_bigram_feat:
                        ...
                        pprev_tag_feat = "B:" + pprev_tag
                        if (pprev_tag_feat, prev_tag) in feat_vec:
                            pprev_tag_weight = prev_tag_weight + feat_vec[pprev_tag_feat, prev_tag]

                    prev_list.append( (prev_tag_weight + prev_value, prev_tag) )
                    pprev_list.append( (pprev_tag_weight + pprev_value, prev_tag, pprev_tag) )

            (best_weight, backpointer) = sorted(prev_list, key=operator.itemgetter(0), reverse=True)[0]
            (bbest_weight, first_backpointer, second_backpointer) = sorted(pprev_list, key=operator.itemgetter(0), reverse=True)[0]
            #print >>sys.stderr, "best_weight:", best_weight, "backpointer:", backpointer
            if best_weight != 0.0 and bbest_weight != 0.0:
                viterbi[i][tag] = (bbest_weight, first_backpointer, second_backpointer)
                found_tag = True
    ...
    for i in range(N-3, 2, -2):
        ...
        (value, backpointer1, backpointer2) = viterbi[i][best_tag]
        output.insert(0, backpointer1)
        best_tag = backpointer2
